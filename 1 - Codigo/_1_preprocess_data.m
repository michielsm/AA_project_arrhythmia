...
I use a slit known as stratified, i.e it's not random but it stands for doing balanced
...
for i=1:number_classes
  ...
  #Overfitting of classes impossible to detect without overfitting (because they are so similar to healthy class)
  if (i == 7 || i==8 || i == 11 || i ==12 || i ==13)
      i_class_train_val = i_class(1:end);
  
      X_train_val = [X_train_val; X(i_class_train_val, :)];
      X_train_val_oversampled = [X_train_val_oversampled; X(i_class_train_val, :)];
      X_test = [X_test; X(i_class_train_val, :)];
    
      Y_train_val = [Y_train_val; Y(i_class_train_val, :)];
      Y_train_val_oversampled = [Y_train_val_oversampled; Y(i_class_train_val, :)];
    ...
    #to achieve the oversampling on these classes
        ...
        if (i != 7 && i!=8 && i != 11 && i != 12 && i != 13) 
         ...
         for (a=1:duplicate_times)
            X_train_val_oversampled = [X_train_val_oversampled; nuevas_filas_x_train_val];
            Y_train_val_oversampled = [Y_train_val_oversampled; nuevas_filas_y_train_val];
         endfor
    ...
    if (i != 7 && i!=8 && i != 11 && i != 12 && i != 13) # && (number_instances_i_class >= 10)
      X_train_val = [X_train_val; X(i_class_train_val, :)];
      X_train_val_oversampled = [X_train_val_oversampled; X(i_class_train_val, :)];
...
save -mat7-binary 'dataset_preprocessed.mat', 'X_train_val', 'Y_train_val', 'X_test', 'Y_test', 'training_val_split_percentage', 'k_folds_cross_val', 'overfitted_classes', 'X_train_val_oversampled', 'Y_train_val_oversampled';
